...
fn main() {
    ...
    let web_package_path = manifest_dir.join("../web/package.json");

    println!("cargo:rerun-if-changed={}", web_package_path.display());

    let web_version = fs::read_to_string(&web_package_path)
        .ok()
