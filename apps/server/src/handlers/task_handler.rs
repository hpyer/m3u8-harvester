...
pub async fn get_tasks(State(state): State<Arc<AppState>>) -> Json<Vec<TaskWithSubtasks>> {
...
pub async fn create_task(
    ...
    let final_year = payload
        .year
        .as_ref()
        .and_then(|y| RE_YEAR.find(y).map(|m| m.as_str().to_string()));

    // 自动优化修复季号格式
    let final_season = payload
        .season
        .as_ref()
        .and_then(|s| RE_SEASON.find(s).map(|m| m.as_str().to_string()));
...
pub async fn delete_completed_tasks(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
