...
use std::sync::Arc;
use m3u8_core::{init_db, TaskService, SettingService, FileService};
use std::env;
use tower_http::{
    cors::CorsLayer,
...
async fn main() {
    ...
    let database_url = env::var("DATABASE_URL")
        .unwrap_or_else(|_| "sqlite:storage/db/app.db".into());
    ...
    let abs_storage_path = std::fs::canonicalize(&storage_path).unwrap_or_else(|_| storage_path.clone());
    ...
    let app = Router::new()
        // API routes
        .route("/api/tasks", get(handlers::task_handler::get_tasks).post(handlers::task_handler::create_task).delete(handlers::task_handler::delete_completed_tasks))
        .route("/api/tasks/:id", get(handlers::task_handler::get_task).delete(handlers::task_handler::delete_task))
        .route("/api/tasks/:id/retry", post(handlers::task_handler::retry_task))
        .route("/api/tasks/:id/pause", post(handlers::task_handler::pause_task))
        .route("/api/tasks/:id/resume", post(handlers::task_handler::resume_task))
        .route("/api/tasks/:id/overwrite", post(handlers::task_handler::respond_overwrite))
        .route("/api/settings", get(handlers::setting_handler::get_settings).post(handlers::setting_handler::update_settings))
        .route("/api/files", get(handlers::file_handler::list_files))
        .route("/api/files/:id", delete(handlers::file_handler::delete_file))
        .route("/api/files/folders/:id", delete(handlers::file_handler::delete_folder))
        .route("/api/files/:id/rename", post(handlers::file_handler::rename_file_or_folder))
