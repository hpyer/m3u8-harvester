...
- ✅ **TMDB 元数据辅助**: 新建任务时可通过 TMDB 搜索电影/剧集信息，自动填充标题、分类、年份，剧集支持 `SxxExx` 自动命名及剧集名称展示。
...
- 下载中的分片和初始化段会写入下载根目录下的 `.temp/<task_id>`。
- 任务成功合并后，会自动清理对应的 `.temp/<task_id>` 目录。
...
storage/
  db/       # SQLite 数据库
  downloads/# 已合并输出的视频文件；下载中的临时文件默认位于 downloads/.temp/
  temp/     # 兼容旧部署保留的运行目录
```
